...
def forces(torque, omega, rb, rl, alpha_tw, betab, Req, Ra, xl, miu, lxi, mu, b):
    ...
    lmin = min(lxi)*b
    if mu == 0:
        COF = 0.048*(np.outer(fbn, 1/vsumc)/(lmin*Req))**0.2 * \
            miu**(-0.05)*Ram**0.25*xl
    else:
...
def hertz(lxi, lsum, bpos, alpha_tw, betab, AE, T1A, T2A, T1T2, rb, E, omega, r, v,
    ...
    vri = np.array(
        [np.outer(omega[0], R1), np.outer(omega[1], R2)])/np.cos(betab)
    R1 = np.tile(R1, (len(bpos), 1)).T
    R2 = np.tile(R2, (len(bpos), 1)).T
    Reff = 1/((1/R1)+(1/R2))/np.cos(betab)
    fnx, a, p0, pm = [np.zeros([len(fbn), len(xx), len(bpos)])
                      for _ in range(4)]
    for i in range(len(fbn)):
        ...
        pm[i] = fnx[i]/(2*a[i])
    p0p = np.array([np.sqrt((2/np.pi)*max(fnx[i, :, 0]) *
                            (Eeff/(2*Req/1000))) for i in range(len(fbn))])
    term = kg*cpg*rohg
    ...
    pvzpx, fax, qvzp1, qvzp2, avg_qvzp1, avg_qvzp2 = [
        np.zeros((len(fbn), len(omega[0]), len(xx))) for _ in range(6)]
    for i in range(len(fbn)):
        for j in range(len(omega[0])):
            ...
            avg_qvzp1[i, j] = qvzp1[i, j]*a[i, :, 0] * \
                omega[0, j]/(np.pi*vri[0, j])
            avg_qvzp2[i, j] = qvzp2[i, j]*a[i, :, 0] * \
                omega[1, j]/(np.pi*vri[1, j])
