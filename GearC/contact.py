...
def forces(torque, omega, rb, rl, alpha_tw, betab, Req, Ra, xl, miu, lxi, mu, b):
    Pin = np.matmul(torque, omega)/2
    fbt = 1000*torque[:, 0]/rb[0, 0]
    ft = 1000*torque[:, 0]/rl[0, 0]
    ...
    frb = fbear/2
    vsumc = 2*omega[0, :]*rl[0, 0]/1000*np.sin(alpha_tw)
    Ram = (Ra[0, 0] + Ra[0, 1])/2
    lmin = min(lxi)*b
    if mu == 0:
...
def lines(betab, epslon_alpha, epslon_beta, epslon_gama, rb, T1A, T2A, AE):
    xx, yy =  [np.linspace(0., 1., 1000) for _ in range(2)]
    lxi = np.zeros(len(xx))
    nn = np.arange(-np.floor(epslon_gama), np.floor(epslon_gama) + 1., 1.)
    xi, xi1, xi2, xi3, li, xxi = [np.zeros((len(xx), len(nn))) for _ in range(6)]
    for i in range(len(nn)):
        ...
        xi3[:, i] = 1*((xi[:, i] - 1) >= 0)
        if betab>0.0: 
            li[:, i] = xi1[:, i]*(xi[:, i] - xi2[:, i]*(xi[:, i] - epslon_beta\
              /epslon_alpha) - xi3[:, i]*(xi[:, i] - 1))/np.sin(betab)
        else:
            li[:, i] = xi1[:, i]
    lxi = li.sum(1)
    rr1 = np.sqrt((xx*AE + T1A)**2 + rb[0, 0]**2)
    rr2 = np.sqrt((T2A - xx*AE)**2 + rb[0, 1]**2)
...
def hertz(lxi, alpha_tw, betab, AE, T1A, T2A, T1T2, rb, E, omega, r, v, fbn, \
    ...
    R = np.zeros((len(xx), 2))
    vt, vri, beta = [np.zeros((2, len(omega[0, :]), len(xx))) for _ in range(3)]
    R[:, 0] = (T1A + xx*AE)/1000
    R[:, 1] = (T2A - xx*AE)/1000
    ...
    for i in range(0, 2):
        for j in range(len(omega[0, :])):
            vt[i, j, :] = omega[i, j]*np.sqrt((rb[0, i]/1000)**2 + R[:, i]**2)
            vri[i, j, :] = omega[i, j]*R[:, i]/np.cos(betab)
            beta[i, j, :] = np.sqrt(kg[i]*cpg[i]*rohg[i]*vri[i, j, :])
    vtb = omega[0, :]*rb[0, 0]/1000
    ...
    for i in range(len(fbn)):
        for j in range(len(omega[0, :])):
            pvzpx[i, j, :] = fnx[i, :]*abs(vg[j, :])*COF[i, j]
