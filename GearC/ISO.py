def LCC(ft,b,m,d,u,v,E,rohg,epslon_alpha,epslon_beta,epslon_gama,beta,\
        betab,alpha,alphat,alpha_tw,n,z1,z2,x1,x2,da,db,df,mat,\
            sigmaHlim,sigmaFlim,niu40,a_l,Rz):
    ...
    FCT2 = pi*d.*cos(beta*pi/180)*cos(alpha*pi/1disp(80)./z.*(epslon_alphan-1);
    FCT3 = (dbn/2).^2;
    ...
    Fact1 = 2*G**2
    Fact2 = cos(vu)*(zn*cos(vu)**2 - 2*G)
    rF = m*(rfP/m + Fact1/Fact2)
    LS = sFn/hFe
    qs = sFn1)/(2*rF(1));
    ...
    sigmaH[0]=ZP*sigmaH0*sqrt(KH);
    sigmaH[1]=ZW*sigmaH0*sqrt(KH);
    SHmin=1.0;
